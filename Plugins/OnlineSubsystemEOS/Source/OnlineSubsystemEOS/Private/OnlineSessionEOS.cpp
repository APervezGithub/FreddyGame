...
{
	EOS_Sessions_RemoveNotifySessionInviteAccepted(EOSSubsystem->SessionsHandle, SessionInviteReceivedId);
	delete SessionInviteReceivedCallback;
...
void FOnlineSessionEOS::Init(const FString& InBucketId)
{
	...
	SessionInviteReceivedCallbackObj->CallbackLambda = [this](const EOS_Sessions_SessionInviteReceivedCallbackInfo* Data)
	{
		...
		if (!NetId.IsValid())
		{
			UE_LOG_ONLINE_SESSION(Warning, TEXT("Cannot receive invite due to unknown user (%s)"), *LexToString(Data->LocalUserId));
			return;
		}
		if (EOSSubsystem->UserManager->GetOnlineUser(Data->TargetUserId) == nullptr)
		{
			UE_LOG_ONLINE_SESSION(Warning, TEXT("Cannot get online user (%s)"), *LexToString(Data->TargetUserId));
			return;
		}
		const FUniqueNetId& OtherNetId = EOSSubsystem->UserManager->GetOnlineUser(Data->TargetUserId)->GetUserId().Get();
		if (!OtherNetId.IsValid())
		{
			UE_LOG_ONLINE_SESSION(Warning, TEXT("Online user is not valid (%s)"), *LexToString(Data->TargetUserId));
			return;
		}
		...
		if (Result == EOS_EResult::EOS_Success)
		{
			...
			//auto otherID = SessionDetails->LocalUserId;
			TriggerOnSessionInviteReceivedDelegates(*NetId, OtherNetId, "OnlineSessionEOS.cpp(492): Not set", LastInviteSearch->SearchResults[0]);
		}
	};
	EOS_Sessions_AddNotifySessionInviteReceivedOptions ReceivedOptions = { };
	ReceivedOptions.ApiVersion = EOS_SESSIONS_ADDNOTIFYSESSIONINVITERECEIVED_API_LATEST;
	SessionInviteReceivedId = EOS_Sessions_AddNotifySessionInviteReceived(EOSSubsystem->SessionsHandle, &ReceivedOptions, SessionInviteReceivedCallbackObj, SessionInviteReceivedCallbackObj->GetCallbackPtr());
	...
	EOS_Sessions_AddNotifySessionInviteAcceptedOptions AcceptedOptions = { };
	AcceptedOptions.ApiVersion = EOS_SESSIONS_ADDNOTIFYSESSIONINVITEACCEPTED_API_LATEST;
	SessionInviteAcceptedId = EOS_Sessions_AddNotifySessionInviteAccepted(EOSSubsystem->SessionsHandle, &AcceptedOptions, SessionInviteAcceptedCallbackObj, SessionInviteAcceptedCallbackObj->GetCallbackPtr());
